...
public class SpringBootKafkaStreamsInitializer
    extends KafkaStreamsInitializer implements ApplicationRunner {
  /**
   ...
   * The server port.
   */
  @Value("${server.port:8080}") private int springBootServerPort;

  /**
   * The Kafka properties.
   */
  @Autowired private KafkaProperties springBootKafkaProperties;

  /**
   * The Kafka Streams starter.
   */
  @Autowired private KafkaStreamsStarter kafkaStreamsStarter;
   ...
   */
  @Override
  protected StreamsUncaughtExceptionHandler.StreamThreadExceptionResponse
  onStreamsUncaughtException(Throwable exception) {
   ...
   */
  @Override
  protected void onStateChange(KafkaStreams.State newState,
  ...
  protected void registerMetrics(KafkaStreams kafkaStreams) {
    // As the Kafka Streams metrics are not picked up by the OpenTelemetry Java
    // agent automatically, register them manually to the Spring Boot registry
    // as the agent will pick metrics up from there
    KafkaStreamsMetrics kafkaStreamsMetrics =
        new KafkaStreamsMetrics(kafkaStreams);
