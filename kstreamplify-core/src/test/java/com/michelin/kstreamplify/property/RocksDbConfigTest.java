...
class RocksDbConfigTest {
  ...
  void testSetConfigWithDefaultValues() {
    ...
    verify(options, times(1))
        .setMaxWriteBufferNumber(
            RocksDbConfig.ROCKSDB_MAX_WRITE_BUFFER_DEFAULT);
    verify(options, times(1))
        .setWriteBufferSize(RocksDbConfig.ROCKSDB_WRITE_BUFFER_SIZE_DEFAULT);
    verify(options, times(1))
  ...
  void testSetConfigWithCustomValues() {
    ...
    Map<String, Object> configs = new HashMap<>();
    configs.put(RocksDbConfig.ROCKSDB_CACHE_SIZE_CONFIG,
                String.valueOf(cacheSize));
    configs.put(RocksDbConfig.ROCKSDB_WRITE_BUFFER_SIZE_CONFIG,
                String.valueOf(writeBufferSize));
    configs.put(RocksDbConfig.ROCKSDB_BLOCK_SIZE_CONFIG,
                String.valueOf(blockSize));
    configs.put(RocksDbConfig.ROCKSDB_MAX_WRITE_BUFFER_CONFIG,
                String.valueOf(maxWriteBuffer));
    configs.put(RocksDbConfig.ROCKSDB_CACHE_INDEX_BLOCK_ENABLED_CONFIG,
                String.valueOf(cacheIndexBlock));
