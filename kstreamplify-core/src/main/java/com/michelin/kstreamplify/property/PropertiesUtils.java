...
public final class PropertiesUtils {
  ...
  public static Properties loadProperties() {
    ...
    try (InputStream inputStream =
             PropertiesUtils.class.getClassLoader().getResourceAsStream(
  ...
  public static Properties loadKafkaProperties(Properties props) {
    ...
    for (var prop : props.entrySet()) {
      if (StringUtils.contains(prop.getKey().toString(),
                               KAFKA_PROPERTIES_PREFIX)) {
        resultProperties.put(
            StringUtils.remove(prop.getKey().toString(),
                               KAFKA_PROPERTIES_PREFIX + PROPERTY_SEPARATOR),
            prop.getValue());
      }
    }
  ...
  parsePropertiesMap(LinkedHashMap<String, Object> map) {
  ...
  private static Properties parseKey(String key, Object map,
    ...
    if (map instanceof LinkedHashMap) {
      for (Object mapKey : ((LinkedHashMap<?, ?>)map).keySet()) {
        parseKey(key + separator + mapKey,
                 ((LinkedHashMap<?, ?>)map).get(mapKey), properties);
      }
    } else {
