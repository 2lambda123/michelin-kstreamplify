...
/**
 * Transformer class for the deduplication mechanism on predicate of a given
 * topic.
 *
...
public class DedupWithPredicateProcessor<K, V extends SpecificRecord>
  ...
  public DedupWithPredicateProcessor(
      String windowStoreName, Duration retentionWindowDuration,
  ...
  public void process(Record<K, V> message) {
    try {
      ...
      // Retrieve all the matching keys in the stateStore and return null if
      // found it (signaling a duplicate)
      try (var resultIterator = dedupWindowStore.backwardFetch(
               identifier, currentInstant.minus(retentionWindowDuration),
        ...
        while (resultIterator != null && resultIterator.hasNext()) {
          ...
          if (identifier.equals(
                  deduplicationKeyExtractor.apply(currentKeyValue.value))) {
            return;
          }
        }
      }

      // First time we see this record, store entry in the window store and
      // forward the record to the output
