...
/**
 * Transformer class for the deduplication mechanism on both keys and values of
 * a given topic.
 *
...
public class DedupKeyValueProcessor<V extends SpecificRecord>
   ...
   */
  public DedupKeyValueProcessor(String windowStoreName,
                                Duration retentionWindowHours) {
  ...
  public void process(Record<String, V> message) {
    try {
      ...
      try (var resultIterator = dedupWindowStore.backwardFetch(
               message.key(), currentInstant.minus(retentionWindowDuration),
