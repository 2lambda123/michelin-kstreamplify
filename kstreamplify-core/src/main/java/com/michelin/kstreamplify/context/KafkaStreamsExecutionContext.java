...
public class KafkaStreamsExecutionContext {
   ...
   * The DLQ topic name.
   */
  @Getter @Setter private static String dlqTopicName;

  /**
   * The Serdes config Map.
   */
  @Getter @Setter private static Map<String, String> serdeConfig;

  /**
   * The properties of the stream execution context.
   */
  @Getter @Setter private static Properties properties;
  ...
  public static void registerProperties(Properties properties) {
    ...
    prefix = properties.getProperty(
        PREFIX_PROPERTY_NAME + PROPERTY_SEPARATOR + SELF, "");
    if (StringUtils.isNotBlank(prefix) &&
      ...
      properties.setProperty(StreamsConfig.APPLICATION_ID_CONFIG,
                             prefix.concat(properties.getProperty(
                                 StreamsConfig.APPLICATION_ID_CONFIG)));
    }

    KafkaStreamsExecutionContext.properties = properties;
    StringBuilder stringBuilderProperties =
        new StringBuilder("Kafka Stream properties:\n");
