...
public final class TopicUtils {
  ...
  /**
   * <p>Prefix the given topic name with the configured prefix and applies the
   * dynamic remap.</p> <p>Prefix is retrieved at runtime from
   * kafka.properties.prefix.[prefixPropertyKey]</p> <pre>{@code kafka:
   *   properties:
   *     prefix:
   *       self: "myNamespacePrefix."
   * }</pre>
   * <p>This allows interactions with multiple topics from different
   * owners/namespaces</p> <p>If not provided, prefixing will not occur.</p>
   * <br/>
   ...
   *
   * @param topicName         The topicName that needs to be prefixed and
   *     remapped
   * @param prefixPropertyKey The prefixPropertyKey matching the configuration
   *     file
   * @return The prefixed and/or remapped topic.
   */
  public static String remapAndPrefix(String topicName,
                                      String prefixPropertyKey) {
    ...
    String resultTopicName = properties.getProperty(
        TOPIC_PROPERTY_NAME + PROPERTY_SEPARATOR + REMAP_PROPERTY_NAME +
            PROPERTY_SEPARATOR + topicName,
        topicName);

    // Check if topic prefix property exists
    String prefix = properties.getProperty(
        PREFIX_PROPERTY_NAME + PROPERTY_SEPARATOR + prefixPropertyKey, "");
