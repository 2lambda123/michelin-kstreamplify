...
public class RocksDbConfig implements RocksDBConfigSetter {
   ...
   */
  public static final String ROCKSDB_CACHE_SIZE_CONFIG =
      "rocksdb.config.cache.size";

  /**
   * The RocksDB write buffer size config key.
   */
  public static final String ROCKSDB_WRITE_BUFFER_SIZE_CONFIG =
      "rocksdb.config.write.buffer.size";

  /**
   * The RocksDB block size config key.
   */
  public static final String ROCKSDB_BLOCK_SIZE_CONFIG =
      "rocksdb.config.block.size";

  /**
   * The RocksDB max write buffer config.
   */
  public static final String ROCKSDB_MAX_WRITE_BUFFER_CONFIG =
      "rocksdb.config.max.write.buffer";

  /**
   * The RocksDB compression type config key.
   */
  public static final String ROCKSDB_COMPRESSION_TYPE_CONFIG =
      "rocksdb.config.compression.type";

  /**
   * The RocksDB cache index block enabled config.
   */
  public static final String ROCKSDB_CACHE_INDEX_BLOCK_ENABLED_CONFIG =
      "rocksdb.config.cache.index.block.enabled";
   ...
   */
  public static final Long ROCKSDB_WRITE_BUFFER_SIZE_DEFAULT =
      4 * ONE_KB * ONE_KB;
  ...
  public void setConfig(final String storeName, final Options options,
    ...
    long blockCacheSize =
        KafkaStreamsExecutionContext.getProperties().containsKey(
            ROCKSDB_CACHE_SIZE_CONFIG)
            ? Long.parseLong(
                  KafkaStreamsExecutionContext.getProperties().getProperty(
                      ROCKSDB_CACHE_SIZE_CONFIG))
    ...
    boolean cacheIndexBlock =
        KafkaStreamsExecutionContext.getProperties().containsKey(
            ROCKSDB_CACHE_INDEX_BLOCK_ENABLED_CONFIG)
    ...
    int maxWriteBuffer =
        KafkaStreamsExecutionContext.getProperties().containsKey(
            ROCKSDB_MAX_WRITE_BUFFER_CONFIG)
            ? Integer.parseInt(
                  KafkaStreamsExecutionContext.getProperties().getProperty(
                      ROCKSDB_MAX_WRITE_BUFFER_CONFIG))
    ...
    String compressionType =
        KafkaStreamsExecutionContext.getProperties().getProperty(
            ROCKSDB_COMPRESSION_TYPE_CONFIG, ROCKSDB_COMPRESSION_TYPE_DEFAULT);
