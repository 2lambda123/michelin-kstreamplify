  ...
  public void start() {
    try {
      server = HttpServer.create(
          new InetSocketAddress(kafkaStreamsInitializer.getServerPort()), 0);
    } catch (IOException e) {
      throw new RuntimeException(e);
    }

    List<HttpEndpoint> httpEndpoints = new ArrayList<>();
    httpEndpoints.add(new HttpEndpoint(
        (String)kafkaStreamsInitializer.getProperties().getOrDefault(
            READINESS_PATH_PROPERTY_NAME, DEFAULT_READINESS_PATH),
        kubernetesService::getReadiness));
    httpEndpoints.add(new HttpEndpoint(
        (String)kafkaStreamsInitializer.getProperties().getOrDefault(
            LIVENESS_PATH_PROPERTY_NAME, DEFAULT_LIVENESS_PATH),
        kubernetesService::getLiveness));
    httpEndpoints.add(new HttpEndpoint(
        (String)kafkaStreamsInitializer.getProperties().getOrDefault(
            TOPOLOGY_PROPERTY, TOPOLOGY_DEFAULT_PATH),
