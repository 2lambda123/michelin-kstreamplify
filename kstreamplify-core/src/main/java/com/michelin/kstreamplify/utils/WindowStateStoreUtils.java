 ...
 *
 * @deprecated Use {@link
 *     com.michelin.kstreamplify.store.WindowStateStoreUtils}.
 */
...
public class WindowStateStoreUtils {
   ...
   */
  public static <K, V> V get(WindowStore<K, V> stateStore, K key,
                             int retentionDays) {
    var resultIterator = stateStore.backwardFetch(
        key, Instant.now().minus(Duration.ofDays(retentionDays)),
        Instant.now());
