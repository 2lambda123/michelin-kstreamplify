 ...
 *
 * @param <K> The model used as the key avro of the topic. Can be String
 *     (Recommended)
...
public class TopicWithSerde<K, V> {
   ...
   * }</pre>
   * <p>If the topic name is {@code myTopic} , at stream initialization the
   * topic name wil resolve to {@code myNamespacePrefix.myTopic}</p>
   */
   ...
   */
  @Getter private final Serde<K> keySerde;

  /**
   * Value serde for the topic.
   */
  @Getter private final Serde<V> valueSerde;

  /**
   * <p>Additional constructor which uses default parameter "self" for
   * prefixPropertyKey.</p>
   *
   * <p>For instance, with the given following configuration :</p>
   * <pre>{@code
   ...
   */
  public TopicWithSerde(String topicName, Serde<K> keySerde,
                        Serde<V> valueSerde) {
   ...
   */
  public String getUnPrefixedName() { return topicName; }

  /**
   * Override of the toString method, dynamically builds the topicName based on
   * springBoot properties for environment/application.
   *
  ...
  /**
   * Wrapper for the .table method of KafkaStreams. Allows simple usage of a
   * topic with type inference
   *
   * @param sb        The streamsBuilder
  ...
  public KTable<K, V> table(StreamsBuilder sb, String storeName) {
    return sb.table(
        this.toString(), Consumed.with(keySerde, valueSerde),
  ...
  /**
   * Wrapper for the .globalTable method of KafkaStreams. Allows simple usage of
   * a topic with type inference
   *
   * @param sb        The streamsBuilder
   * @param storeName The StoreName
  ...
  /**
   * Wrapper for the .to method of Kafka streams. Allows simple usage of a topic
   * with type inference
   *
