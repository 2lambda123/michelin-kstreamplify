...
public class ProcessingResult<V, V2> {
   ...
   */
  private ProcessingResult(V value) { this.value = value; }
   ...
   */
  private ProcessingResult(ProcessingError<V2> error) { this.error = error; }
   ...
   * <p>The resulting stream needs to be handled with
   * TopologyErrorHandler#catchErrors(KStream) for automatic DLQ redirection on
   * failed records.</p>
   *
   * @param message The resulting successful Record from the processor that
   *     needs to be wrapped in a ProcessingResult
   ...
   */
  public static <K, V, V2> Record<K, ProcessingResult<V, V2>>
  wrapRecordSuccess(Record<K, V> message) {
    return new Record<>(message.key(),
  ...
  /**
   * <p>
   * Wraps a key, value and timestamp in a Record with
   * ProcessingResult#success(V value) as value.
   * </p>
   * <p>
   * The resulting stream needs to be handled with
   * TopologyErrorHandler#catchErrors(KStream) for automatic DLQ redirection on
   * failed records.
   * </p>
   *
   ...
   */
  public static <K, V, V2> Record<K, ProcessingResult<V, V2>>
  wrapRecordSuccess(K key, V value, long timestamp) {
   ...
   * {@link ProcessingResult#wrapRecordFailure(Exception, Record)} for easier
   * syntax.
   * </p>
   *
   ...
   */
  public static <V, V2> ProcessingResult<V, V2> fail(Exception e,
                                                     V2 failedRecordValue) {
   ...
   * @param e                 The exception
   * @param failedRecordValue The failed Kafka record
   * @param contextMessage    The custom context message that will be added in
   *     the stack trace
   * @param <V>               The type of the successful record
   * @param <V2>              The type of the failed record
   * @return A processing result containing the failed record
   */
  public static <V, V2> ProcessingResult<V, V2>
  fail(Exception e, V2 failedRecordValue, String contextMessage) {
    return new ProcessingResult<>(
        new ProcessingError<>(e, contextMessage, failedRecordValue));
  }

  /**
   * <p>
   * Wraps a record's value with {@link ProcessingResult#fail(Exception,
   * Object)} )}.
   * </p>
   * <p>
   * The resulting stream needs to be handled with
   ...
   *                     the processor that needs to be wrapped in a {@link
   * ProcessingResult}
   * @param <K>          The type of the record key
   * @param <V>          The type of the ProcessingResult successful value
   * @param <V2>         The type of the ProcessingResult error value
   * @return The initial Record, with value wrapped in a {@link ProcessingError}
   *     and {@link ProcessingResult}
   */
  public static <K, V, V2> Record<K, ProcessingResult<V, V2>>
  wrapRecordFailure(Exception e, Record<K, V2> failedRecord) {
   ...
   *                       the processor that needs to be wrapped in a {@link
   * ProcessingResult}
   * @param contextMessage The custom context message that will be added in the
   *     stack trace
   * @param <K>            The type of the record key
   * @param <V>            The type of the ProcessingResult successful value
   * @param <V2>           The type of the ProcessingResult error value
  ...
  /**
   * <p>Wraps a key, value and timestamp in a Record with {@link
   * ProcessingResult#fail(Exception, Object, String)} as value.</p> <p>The
   * resulting stream needs to be handled with
   * TopologyErrorHandler#catchErrors(KStream) for automatic DLQ redirection on
   * failed records.</p>
   *
   * @param e           The initial exception
   ...
   * @param <V>         The type of the ProcessingResult successful value
   * @param <V2>        The type of the ProcessingResult error value
   * @return A Record with value wrapped in a {@link ProcessingError} and {@link
   *     ProcessingResult}
   */
  public static <K, V, V2> Record<K, ProcessingResult<V, V2>>
  wrapRecordFailure(Exception e, K key, V2 failedValue, long timestamp) {
   ...
   *                       that needs to be wrapped in a {@link
   * ProcessingResult}
   * @param timestamp      The timestamp to apply on the resulting record
   * @param contextMessage The custom context message that will be added in the
   *     stack trace
  ...
  wrapRecordFailure(Exception e, K key, V2 failedValue, long timestamp,
                    String contextMessage) {
    return new Record<>(
        key, ProcessingResult.fail(e, failedValue, contextMessage), timestamp);
  }
   ...
   */
  public boolean isValid() { return value != null && error == null; }
}
