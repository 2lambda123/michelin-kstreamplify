...
public class DlqDeserializationExceptionHandler
    extends DlqExceptionHandler implements DeserializationExceptionHandler {
  ...
  public DlqDeserializationExceptionHandler(
      Producer<byte[], KafkaError> producer) {
  ...
  handle(ProcessorContext processorContext,
         ConsumerRecord<byte[], byte[]> consumerRecord,
         Exception consumptionException) {
    if (StringUtils.isBlank(KafkaStreamsExecutionContext.getDlqTopicName())) {
      log.warn("Failed to route deserialization error to the designated DLQ " +
    ...
    try {
      ...
      enrichWithException(builder, consumptionException, consumerRecord.key(),
          ...
          .setContextMessage("An exception occurred during the stream " +
                             "internal deserialization")
          .setOffset(consumerRecord.offset())
          .setPartition(consumerRecord.partition())
          .setTopic(consumerRecord.topic());

      boolean isCausedByKafka =
          consumptionException.getCause() instanceof KafkaException;
      // If the cause of this exception is a KafkaException and if getCause ==
      // sourceException (see Throwable.getCause - including
      // SerializationException) use to handle poison pill => sent message into
      // dlq and continue our life.
      if (isCausedByKafka || consumptionException.getCause() == null) {
        producer
            .send(new ProducerRecord<>(
                KafkaStreamsExecutionContext.getDlqTopicName(),
                consumerRecord.key(), builder.build()))
            .get();
        return DeserializationHandlerResponse.CONTINUE;
      }
    } catch (InterruptedException ie) {
      log.error("Interruption while sending the deserialization exception {} " +
                "for key {}, "
                    + "value {} and topic {} to DLQ topic {}",
                consumptionException, consumerRecord.key(),
                consumerRecord.value(), consumerRecord.topic(),
                KafkaStreamsExecutionContext.getDlqTopicName(), ie);
      Thread.currentThread().interrupt();
    } catch (Exception e) {
      log.error("Cannot send the deserialization exception {} for key {}, " +
                "value {} and topic {} to DLQ topic {}",
                consumptionException, consumerRecord.key(),
                consumerRecord.value(), consumerRecord.topic(),
    ...
    // here we only have exception like UnknownHostException for example or
    // TimeoutException ... situation example:  we cannot ask schema registry
    // because the url is unavailable
    return DeserializationHandlerResponse.FAIL;
  }
  ...
  public void configure(Map<String, ?> configs) {
    synchronized (GUARD) {
      if (producer == null) {
        instantiateProducer(DlqDeserializationExceptionHandler.class.getName(),
                            configs);
      }
    }
  }
