...
public class TopologyErrorHandler {
   ...
   *
   * @param stream The stream of processing result that may contain processing
   *     errors
   ...
   */
  public static <K, V, V2> KStream<K, V>
  catchErrors(KStream<K, ProcessingResult<V, V2>> stream) {
   ...
   * Catch the errors from the given stream.
   *
   * @param stream         The stream of processing result that may contain
   *     processing errors
   * @param allowTombstone Allow sending tombstone in DLQ topic or to be
   *     returned
   * @param <K>            The key type
   * @param <V>            The type of the successful record
   * @param <V2>           The type of the failed record
  ...
  catchErrors(KStream<K, ProcessingResult<V, V2>> stream,
    ...
    if (!allowTombstone) {
      branches = stream.filter((key, value) -> value != null)
                     ...
                     .defaultBranch(Branched.withConsumer(
                         ks
                         -> TopologyErrorHandler.handleErrors(
                             ks.mapValues(ProcessingResult::getError))));
    } else {
      branches = stream.filter((key, value) -> value != null)
                     .split(Named.as(branchNamePrefix))
                     .branch((key, value)
    ...
    return branches.get(branchNamePrefix + BRANCHING_NAME_NOMINAL)
        .mapValues(ProcessingResult::getValue);
  }

  /**
   * Process a stream of processing errors and route it to the configured DLQ
   * topic.
   *
  ...
  handleErrors(KStream<K, ProcessingError<V>> errorsStream) {
    if (StringUtils.isBlank(KafkaStreamsExecutionContext.getDlqTopicName())) {
      log.warn("Failed to route topology error to the designated DLQ (Dead " +
               "Letter Queue) topic. "
    ...
    errorsStream
        .map(
            (key, value)
                -> new KeyValue<>(key == null ? "null" : key.toString(), value))
        .processValues(GenericErrorProcessor<V>::new)
        .to(KafkaStreamsExecutionContext.getDlqTopicName(),
