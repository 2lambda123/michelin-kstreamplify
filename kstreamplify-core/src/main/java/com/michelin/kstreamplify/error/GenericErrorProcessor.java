 ...
 */
class GenericErrorProcessor<V>
    extends ContextualFixedKeyProcessor<String, ProcessingError<V>,
                                        KafkaError> {
  /**
   * Process the error.
   *
  ...
  @Override
  public void
  process(FixedKeyRecord<String, ProcessingError<V>> fixedKeyRecord) {
    ...
    KafkaError error =
        KafkaError.newBuilder()
            ...
            .setPartition(recordMetadata != null ? recordMetadata.partition()
                                                 : -1)
