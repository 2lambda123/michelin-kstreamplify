...
public final class KubernetesService {
   ...
   */
  public static final String READINESS_PATH_PROPERTY_NAME =
      "kubernetes.readiness.path";

  /**
   * The liveness path property name.
   */
  public static final String LIVENESS_PATH_PROPERTY_NAME =
  ...
  public RestResponse<Void> getReadiness() {
    if (kafkaStreamsInitializer.getKafkaStreams() != null) {
      ...
      if (kafkaStreamsInitializer.getKafkaStreams().state() ==
        ...
        long startingThreadCount =
            kafkaStreamsInitializer.getKafkaStreams()
                ...
                .filter(
                    t
                    -> StreamThread.State.STARTING.name().compareToIgnoreCase(
                           t.threadState()) == 0 ||
                           StreamThread.State.CREATED.name()
                                   .compareToIgnoreCase(t.threadState()) == 0)
                .count();

        if (startingThreadCount == kafkaStreamsInitializer.getKafkaStreams()
                                       .metadataForLocalThreads()
                                       .size()) {
      ...
      return kafkaStreamsInitializer.getKafkaStreams().state().equals(
