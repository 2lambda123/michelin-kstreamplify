...
public class InteractiveQueriesService {
  ...
  @Getter private final KafkaStreamsInitializer kafkaStreamsInitializer;
   ...
   */
  public InteractiveQueriesService(
  ...
  public Collection<StreamsMetadata> getStreamsMetadata(final String store) {
    return kafkaStreamsInitializer.getKafkaStreams().streamsMetadataForStore(
        store);
  }
   ...
   * @return The values
   */
  public List<QueryResponse> getAll(String store, boolean includeKey,
                                    boolean includeMetadata) {
    final Collection<StreamsMetadata> streamsMetadata =
        getStreamsMetadata(store);

    if (streamsMetadata == null || streamsMetadata.isEmpty()) {
      throw new UnknownStateStoreException(
          String.format(UNKNOWN_STATE_STORE, store));
    }

    List<QueryResponse> values = new ArrayList<>();
    streamsMetadata.forEach(metadata -> {
      if (isNotCurrentHost(metadata.hostInfo())) {
        log.debug("Fetching data on other instance ({}:{})", metadata.host(),
                  metadata.port());

        List<QueryResponse> queryResponses = requestAllOtherInstance(
            metadata.hostInfo(), "/store/" + store +
                                     "?includeKey=" + includeKey +
                                     "&includeMetadata=" + includeMetadata);
        values.addAll(queryResponses);
      } else {
        log.debug("Fetching data on this instance ({}:{})", metadata.host(),
                  metadata.port());

        RangeQuery<Object, ValueAndTimestamp<Object>> rangeQuery =
            RangeQuery.withNoBounds();
        StateQueryResult<KeyValueIterator<Object, ValueAndTimestamp<Object>>>
            result = kafkaStreamsInitializer.getKafkaStreams().query(
        ...
        result.getPartitionResults().forEach(
            (key,
             queryResult) -> queryResult.getResult().forEachRemaining(kv -> {
              QueryResponse queryResponse;
              if (includeMetadata) {
                Set<String> topics = queryResult.getPosition().getTopics();
                List<QueryResponse.PositionVector> positions =
                    topics.stream()
                        .flatMap(
                            topic
                            -> queryResult.getPosition()
                                   ...
                                   .stream()
                                   .map(partitionOffset
                                        -> new QueryResponse.PositionVector(
                                            topic, partitionOffset.getKey(),
                                            partitionOffset.getValue())))
                        .toList();

                queryResponse = new QueryResponse(
                    includeKey ? kv.key : null, kv.value.value(),
                    kv.value.timestamp(),
                    new HostInfoResponse(metadata.hostInfo().host(),
   ...
   * @return The value
   */
  public <K> QueryResponse getByKey(String store, K key,
                                    Serializer<K> serializer,
                                    boolean includeKey,
                                    boolean includeMetadata) {
    KeyQueryMetadata keyQueryMetadata =
        getKeyQueryMetadata(store, key, serializer);
    ...
    if (isNotCurrentHost(host)) {
      ...
      return requestOtherInstance(
          host, "/store/" + store + "/" + key + "?includeKey=" + includeKey +
                    "&includeMetadata=" + includeMetadata);
    }

    log.debug("The key {} has been located on the current instance ({}:{})",
              key, host.host(), host.port());

    KeyQuery<K, ValueAndTimestamp<Object>> keyQuery = KeyQuery.withKey(key);
    StateQueryResult<ValueAndTimestamp<Object>> result =
        kafkaStreamsInitializer.getKafkaStreams().query(
            StateQueryRequest.inStore(store).withQuery(keyQuery).withPartitions(
                Collections.singleton(keyQueryMetadata.partition())));
    ...
    if (includeMetadata) {
      Set<String> topics =
          result.getOnlyPartitionResult().getPosition().getTopics();
      List<QueryResponse.PositionVector> positions =
          topics.stream()
              .flatMap(topic
      ...
      return new QueryResponse(
          ...
          new HostInfoResponse(host.host(), host.port()), positions);
    }
   ...
   * @return The host
   */
  private <K> KeyQueryMetadata getKeyQueryMetadata(String store, K key,
                                                   Serializer<K> serializer) {
    return kafkaStreamsInitializer.getKafkaStreams().queryMetadataForKey(
        store, key, serializer);
  }
   ...
   * @return The response
   */
  private List<QueryResponse> requestAllOtherInstance(HostInfo host,
                                                      String endpointPath) {
    try {
   ...
   * @return The response
   */
  private QueryResponse requestOtherInstance(HostInfo host,
                                             String endpointPath) {
    try {
      String jsonResponse = sendRequest(host, endpointPath);
    ...
    try {
      HttpRequest request =
          HttpRequest.newBuilder()
              ...
              .uri(new URI(String.format("http://%s:%d/%s", host.host(),
  ...
  private boolean isNotCurrentHost(HostInfo compareHostInfo) {
    return !kafkaStreamsInitializer.getHostInfo().host().equals(
               compareHostInfo.host()) ||
