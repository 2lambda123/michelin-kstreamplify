 ...
 * <p>It provides a {@link TopologyTestDriver} and a {@link TestOutputTopic} for
 * the DLQ</p>
 */
public abstract class KafkaStreamsStarterTest {
  ...
  void generalSetUp() {
    ...
    properties.setProperty(StreamsConfig.STATE_DIR_CONFIG,
                           STATE_DIR + getClass().getName());

    KafkaStreamsExecutionContext.registerProperties(properties);
    KafkaStreamsExecutionContext.setSerdeConfig(Collections.singletonMap(
  ...
  /**
   * Creates an input test topic on the testDriver using the provided
   * topicWithSerde.
   *
   ...
   */
  protected <K, V> TestInputTopic<K, V>
  createInputTestTopic(TopicWithSerde<K, V> topicWithSerde) {
  ...
  /**
   * Creates an output test topic on the testDriver using the provided
   * topicWithSerde.
   *
   * @param topicWithSerde The topic with serde used to crete the test topic
   * @param <K>            The serializable type of the key
   ...
   */
  protected <K, V> TestOutputTopic<K, V>
  createOutputTestTopic(TopicWithSerde<K, V> topicWithSerde) {
